...
import {enterEmphasis} from "echarts/types/src/util/states";
...
const instance = Axios.create({
    ...
    timeout: TIME_OUT,
    headers: {
        Authorization: "token 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",
    }
})
...
const getPromise = (method, url, params?, config = {}): Promise<any> => {
    return new Promise((resolve, reject) => {
        if (method === 'get') {
            ...
            instance.post(url, params, config).catch(e => e)
                .then(res => resolve(res))
                .catch(err => reject(err))
        }
    })
